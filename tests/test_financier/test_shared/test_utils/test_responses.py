...
class TestResponses(TestCase):
    def test_make_response(self):
        self.assertEqual(responses.make_response({'test': 'test'}, HTTPStatus.OK), {
            'statusCode': 200,
    ...
    def test_make_response_no_content(self):
        self.assertEqual(responses.make_response({}, HTTPStatus.NO_CONTENT), {
            'statusCode': 204,
            'headers': {'Content-Type': 'application/json'},
            'isBase64Encoded': False,
            'multiValueHeaders': {},
    ...
    def test_make_error_response(self):
        self.assertEqual(responses.make_error_response(), {
            'statusCode': 500,
            'headers': {
    ...
    def test_make_error_response_custom(self):
        self.assertEqual(responses.make_error_response(body='test'), {
            'statusCode': 500,
            'headers': {
                'Content-Type': 'application/json',
                'x-amzn-ErrorType': 500,
    ...
    def test_make_list_response(self):
        self.assertEqual(responses.make_list_response([1, 2, 3], 'test', HTTPStatus.OK), {
            'statusCode': 200,
            'headers': {'Content-Type': 'application/json'},
            'isBase64Encoded': False,
            'multiValueHeaders': {},
