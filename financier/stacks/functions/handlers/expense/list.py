...
def handler(event, context):
    keys = extract_multi_value_query_parameters(event).get(
        QUERY_PARAM_LAST_EVALUATED_KEY, None
    )
    last_evaluated_key = None
    if keys:
        last_evaluated_key = compose_last_evaluated_key(keys)
    items = ExpenseModel.query(
        hash_key=DEFAULT_SYSTEM_ID, last_evaluated_key=last_evaluated_key
    )
    iterated_items = []
    limit = 10
    ...
    return make_list_response(
        items=iterated_items, last_evaluated_key=items.last_evaluated_key
    )
...
def generate_key(user_id: str, created_at: str) -> dict:
    return {
        ExpenseModel.user_id.attr_name: {"S": user_id},
        ExpenseModel.created_at.attr_name: {"S": created_at},
    }
